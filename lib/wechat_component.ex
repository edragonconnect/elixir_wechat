...
defmodule WeChat.Component.Base.Local do
  ...
  defp refresh_or_refetch_token_to_refresh(appid, authorizer_appid, authorizer_refresh_token) do
    refresh_result = remote_refresh_authorizer_access_token(appid, authorizer_appid, authorizer_refresh_token)
  ...
  defp remote_refresh_authorizer_access_token(appid, authorizer_appid, authorizer_refresh_token) do
    ...
    request_result = WeChat.request(:post, url: "/cgi-bin/component/api_authorizer_token", body: data, appid: appid)
  ...
  defp remote_find_authorizer_refresh_token(appid, authorizer_appid, offset \\ 0, count \\ 500) do
    ...
    request_result = WeChat.request(:post, url: "/cgi-bin/component/api_get_authorizer_list", body: %{"offset" => offset, "count" => count})
        ...
        if matched != nil do
          ...
          if size_in_list == 0 or size_in_list == total_count do
            ...
            if offset + 1 < total_count do
              remote_find_authorizer_refresh_token(appid, authorizer_appid, offset + size_in_list, count)
            else
              Logger.error("not find matched authorizer_appid: #{authorizer_appid} in authorizer_list, please double check.")
              raise %Error{reason: :invalid_authorizer_appid}
            end
...
defmodule WeChat.Component.Base.Hub do
  ...
  defp remote_get_component_access_token(appid, adapter_storage) do
    ...
    request_result =
      WeChat.request(:post, url: "/cgi-bin/component/api_component_token", body: %{"verify_ticket" => verify_ticket}, query: [appid: appid])
