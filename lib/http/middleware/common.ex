defmodule WeChat.Http.Middleware.Common do
  ...
  defp try_use_storage_cache(env, %Request{uri: %URI{path: "/cgi-bin/ticket/getticket"}} = request) do
  ...
  defp populate_access_token(env, %Request{uri: %URI{path: "/cgi-bin/token"}, appid: appid, adapter_storage: adapter_storage} = request) do
    prepared_query = [
  ...
  defp populate_access_token(env, %Request{access_token: nil, authorizer_appid: nil, appid: appid, adapter_storage: adapter_storage} = request) do
  ...
  defp populate_access_token(env, %Request{access_token: nil, authorizer_appid: authorizer_appid, appid: appid, adapter_storage: adapter_storage} = request) when authorizer_appid != nil do
  ...
  defp sync_to_storage_cache(
    ...
    %Request{uri: %URI{path: "/cgi-bin/token"}, adapter_storage: adapter_storage, appid: appid}) do
      adapter_storage.save_access_token(appid, access_token)
  end
  defp sync_to_storage_cache(
    %{"ticket" => ticket},
    %Request{uri: %URI{path: "/cgi-bin/ticket/getticket"}, use_case: :hub, query: query,
      adapter_storage: adapter_storage, appid: appid, authorizer_appid: authorizer_appid})
  ...
  defp sync_to_storage_cache(
    %{"ticket" => ticket},
    %Request{uri: %URI{path: "/cgi-bin/ticket/getticket"}, use_case: :hub, query: query,
      adapter_storage: adapter_storage, appid: appid, authorizer_appid: nil}) do
      type = Keyword.get(query, :type)
  ...
  defp rerun_when_token_expire(env, next, %Request{appid: appid, authorizer_appid: authorizer_appid, adapter_storage: adapter_storage} = request, %{"errcode" => errcode}, request_query)
    when errcode == 40001
    when errcode == 42001
  ...
  defp rerun_when_token_expire(_env, _next, request, %{"errcode" => errcode} = json_resp_body, _request_query) when errcode == 61024 do
    Logger.error "invalid usecase to get access_token of authorizer appid(#{request.authorizer_appid}) by WeChat Component Application(#{request.appid})"
    {:error, %Error{reason: :invalid_usecase_get_access_token, errcode: errcode, message: Map.get(json_resp_body, "errmsg")}}
  end
