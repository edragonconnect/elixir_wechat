defmodule WeChat.IntegrationTest do
  ...
  test "user/info" do
    {:ok, response} = TestClient1.request(:get, url: "/cgi-bin/user/info", appid: "wx6973a7470c360256")
    ...
    {:ok, response} = TestClient1.request(:get, url: "/cgi-bin/user/info", appid: "wx6973a7470c360256", query: query)
  ...
  test "media/upload - file" do
    ...
    body = %{media: media}
    {:ok, response} = TestClient1.request(:post, url: "/cgi-bin/media/upload", body: {:form, body}, appid: "wx6973a7470c360256")
    assert Map.get(response.body, "media_id") != nil

    {:ok, response2} = TestClient2.request(:post, url: "/cgi-bin/media/upload", body: {:form, body})
  ...
  test "batchget_material" do
    ...
    {:ok, response} = TestClient1.request(:post, url: "/cgi-bin/material/batchget_material", body: body, appid: "wx6973a7470c360256")
    material_items = Map.get(response.body, "item")
    assert is_list(material_items) == true
    assert length(material_items) <= batchget_size

    {:ok, response2} = TestClient2.request(:post, url: "/cgi-bin/material/batchget_material", body: body)
    assert response2.body == response.body

    # `authorizer_appid` can be override when dynamically send request
    {:ok, response3} = TestClient2.request(:post, url: "/cgi-bin/material/batchget_material", body: body, appid: "wx6973a7470c360256")
    assert response3.body == response.body

    {:error, error} = TestClient2.request(:post, url: "/cgi-bin/material/batchget_material", body: body, appid: "fake_authorizer_appid")
